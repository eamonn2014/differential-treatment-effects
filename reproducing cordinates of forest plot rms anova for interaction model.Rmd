---
title: "(i) Show how coordinates of forest plot can be found for interaction model by scaling the regression coeficients and stbadard error by duplicating R rms summary function and (ii) duplicating anova.rms function"
author: "Eamonn"
...
```{r effect2, include=TRUE, echo=TRUE}
  ...
  var <- vcov(A)   # get the variance covariance matrix, standard error (se) is here or
  #get it from regression table
  
  (diff <- upp-low)  # range
  
  # rescale the effect of variable on log odds of response,
  # the coefficient is over 1 unit by default in regression table
  (eff <- coef(A)[variable][[1]]*diff)   
 
  se <-  diff*sqrt(diag(var)[variable][[1]])  # rescale standard error from model sqrt covariance , 
  #or se from reg table
...
```{r anova0, include=TRUE, echo=TRUE}
    ...
    # select only information of interest, show matrix
    cov[idx, idx]
...
```{r anova1, include=TRUE, echo=TRUE  }
   ...
   res <- matrix(, nrow = length(labels), ncol = 4)
     ...
     for ( i in 1 : length(namez))  {
        # select parameters we are interested in
       par <- namez[i]                   
     ...
     # do the calculations.
     # solvet is function solves the equation a %*% x = b for x, 
     # where b can be either a vector or a matrix. a is square
     (chi <- co %*% solvet( c1, co ))  # chi square value
     (df. <- length(idx))              # degrees of freedom
     (pvalue <-(1 - pchisq(chi, df.))) # pvalue
...
```{r anova2, include=TRUE, echo=TRUE, results='asis'}
...
```{r anova3, include=TRUE, echo=TRUE}    
    ...
    # extract coefficients only with * in name
    c1 <-  cov[(idx),(idx)]                                       
    co <- A$coefficients[idx] 
